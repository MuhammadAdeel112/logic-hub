...
class MyCompletedJobWidget extends StatefulWidget {
  const MyCompletedJobWidget({
    Key? key,
  }) : super(key: key);

  @override
  State<MyCompletedJobWidget> createState() => _MyCompletedJobWidgetState();
}

class _MyCompletedJobWidgetState extends State<MyCompletedJobWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    // final waitingforreview=assignedJobsProvider.assignedJobs.

    // Get the JobSearchProvider instance
    final jobSearchProvider = Provider.of<JobSearchProvider>(context);
    final initialJobList =
        assignedJobs?.where((job) => job.approvalStatus== 'pending'&& job.status !='pending').toList();
          ...
          case Status.COMPLETED:
            if (initialJobList == null || initialJobList.isEmpty) {
              return ResueAbleErrorScreen(
                errorMsg:
                    'No Completed jobs yet. Once you complete a job, it will appear here.',
              );
            }
